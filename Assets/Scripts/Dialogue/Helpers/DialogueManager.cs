...
namespace Assets.Scripts.Dialogue.Helpers
{
	public class DialogueManager : MonoBehaviour
	{
		...
		private string path = @""; //указать путь до файла с диалогом
        ...
        public static DialogueManager Internal { get; private set; }
		...
		void Load()
		{
			...
			{
				var binary = Resources.Load<TextAsset>(path + @"\" + fileName);
				var reader = new XmlTextReader(new StringReader(binary.text));
				var index = 0;
				while (reader.Read())
				{
